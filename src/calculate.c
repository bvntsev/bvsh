...
static uint64_t *check_correct_expr(const char *expr, uint64_t *len) {
    ...
    uint64_t *expr_temp_data = malloc(sizeof(uint64_t) * 3);
#define EXPR_CODE_ERROR         expr_temp_data[0]
#define EXPR_OPERATORS_N        expr_temp_data[1]
#define EXPR_BRACKETS_N         expr_temp_data[2]
    char expr_code = 0;
    uint64_t error_position = -1;
    for (uint64_t iter = 0; iter < *len-1; ++iter) {
        switch (expr[iter]){
            case '0': break; // verify that isn't first symbol i guess
            case '1': break;
            case '2': break;
            case '3': break;
            case '4': break;
            ...
            case '(': { //FIX: )( ( '(' ++ and ')' --, check depth == 0 
                        //if i found ')')
                EXPR_BRACKETS_N++; break;
            }
            case ')': {
                EXPR_BRACKETS_N++; break;
            }
...
struct calc *parsing_str(const char *expr, uint64_t *len) {
    ...
    calc->stack_operations_n = EXPR_OPERATORS_N; 
    calc->stack_brackets_n = EXPR_BRACKETS_N;
    calc->value = 0;
